...
- **`Huia.OpenId`** — flow-aware Identity managers. `HuiaAuthFlow`
...
- **`Huia.OpenId`** — **Breaking.** the custom `HuiaUserConfirmation : IUserConfirmation<HuiaUser>`
  ...
  projection instead.
- **`Huia` / `Huia.OpenId`** — **Breaking.** lockout is per **flow**, per tenant, not per tenant.
...
- **`Huia.OpenId`** — the "forgot password" page now carries `returnUrl` through to its "Sign in"
...
- **`Huia.OpenId`** — the "you've signed in from this number very recently" limit is now checked on
  ...
  sign-in.
- **`Huia.OpenId`** — signing out of a session that was established through an external provider now
...
- **`Huia.OpenId`** — the account-UI error alert no longer sits flush against the text above it
...
- **`Huia`** — **Breaking.** `HuiaClientDescriptor.RequirePushedAuthorizationRequests` is now a fluent
  ...
  unchanged.
- **`Huia` / `Huia.OpenId`** — password-complexity policy is now per tenant.
...
- **`Huia.OpenId`** — the account UI renders the tenant's branding: the `_Layout` shows
...
- **`Huia.OpenId`** — after an interactive sign-in reached with no OAuth flow in progress (no
  ...
  (`TenantClientHome`).
- **`Huia.OpenId`** — the committed account-UI assets under `wwwroot/` are now regenerated by an
  MSBuild target (`BuildAccountUiAssets` in `Huia.OpenId.csproj`) that runs the Tailwind CLI and
...
- **`Huia.OpenId`** — the `/admin` list rows for clients and scopes now carry an `origin` field:
...
- **`Huia.OpenId.EntityFrameworkCore`** — tenant isolation is now enforced by Finbuckle instead of bespoke
  ...
  moved to the `Huia.OpenId.EntityFrameworkCore.Multitenancy` namespace. Adopters on a real EF Core provider
...
- **`Huia.OpenId`** — interactive login sessions are now bound to their tenant:
...
- **`Huia.OpenId`** — `RouteOptions.LowercaseUrls` is enabled, so generated links (including the
  Razor account-UI page links) are lower-case.
- **`Huia.OpenId`** — the security-headers `form-action` CSP directive now also allows the
...
- **`Huia.OpenId`** — `MapHuiaHome()` no longer redirects a tenant root (`/{tenant}/`) back to
...
- **`Huia.OpenId`** — account-UI forms have a consistent vertical rhythm again: Basecoat's `.form`
...
- **`Huia.OpenId`** — `/connect/logout` without a resolvable client (for example a relying party
...
- **`Huia.OpenId`** — the redesigned one-time-code verification page (`VerifyOtp`): tightened
...
- **`Huia.OpenId`** — the `/admin/*` API gains full CRUD for users (`GET/POST/PUT/DELETE
...
- **`Huia` / `Huia.OpenId`** — the resource-owner-password-credentials (`password`) grant is
...
- **`Huia.OpenId`** — the standalone `/identity/account/phonelogin` page is gone; the phone
...
- **`Huia` / `Huia.OpenId`** — Pushed Authorization Requests (RFC 9126). The server exposes
...
- **`Huia.OpenId`** — `UseHuia()` now installs `UseStatusCodePagesWithReExecute`, so bare
...
- **CI** — `release.yml` publishes the NuGet packages (`Huia`, `Huia.OpenId.EntityFrameworkCore`,
  `Huia.OpenId` + symbols) on a `v*.*.*` tag or `workflow_dispatch`, with the version derived by
...
- **`Huia.OpenId`** — `IPhoneLoginRateLimiter` (default in-memory `System.Threading.RateLimiting`
...
- **`Huia.OpenId.EntityFrameworkCore`** — `HuiaDbContext : IdentityDbContext<HuiaUser, HuiaRole, string>` with
...
- **`Huia.OpenId`** — `AddHuia()` / `UseHuia()` with the load-bearing middleware order; Finbuckle
