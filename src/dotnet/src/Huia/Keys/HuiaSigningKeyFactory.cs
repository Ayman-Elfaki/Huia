...
internal sealed class HuiaSigningKeyFactory(IHuiaKeyProtector protector, TimeProvider timeProvider)
{
    ...
    /// <returns>The persistable key record.</returns>
    public HuiaSigningKeyRecord Create(string tenantId, KeyManagementOptions options, HuiaSigningKeyStatus status, DateTimeOffset activateAt)
    {
        ...
        return new HuiaSigningKeyRecord(
            Guid.NewGuid().ToString("N"),
            tenantId,
            keyId,
            SecurityAlgorithms.RsaSha256,
            JsonSerializer.Serialize(publicJwk),
            protector.Protect(rsa.ExportPkcs8PrivateKey()),
            status,
