...
/// <see cref="AuthorizationPolicyBuilder.RequireAuthenticatedUser"/> and pins the Huia API scheme once,
/// so an anonymous call is a 401 bearer challenge rather than a redirect to the login page.
/// </summary>
public static class HuiaAuthorizationPolicyBuilderExtensions
{
    ...
    {
        if (!builder.AuthenticationSchemes.Contains(HuiaConstants.Schemes.Api))
        {
            builder.AddAuthenticationSchemes(HuiaConstants.Schemes.Api);
