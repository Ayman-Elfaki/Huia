...
/// <summary>
/// The Huia <see cref="SignInManager{TUser}"/>, shared by every Huia flavor. Registered via
/// <c>.AddSignInManager&lt;TSignInManager&gt;()</c> so every <see cref="SignInManager{TUser}"/> resolution
/// gets it, and constructed directly by a flow-identity factory with a flow-specific
/// <see cref="IdentityOptions"/> instance where the host has one. It is a thin seam over the framework:
/// the one added member is the discoverable passkey sign-in.
/// </summary>
/// <remarks>Creates the manager. Parameters are the stock <see cref="SignInManager{TUser}"/> dependencies.</remarks>
public class HuiaSignInManager<TUser>(
    ...
    where TUser : class
{
    ...
    public async Task<(SignInResult Result, TUser? User)> PasskeyPrimarySignInAsync(string credentialJson, bool isPersistent)
    {
