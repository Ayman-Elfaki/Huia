...
/// blank-name <see cref="Huia.Identity.HuiaUser"/> at request time. The default
...
public interface IPendingPhoneSignup
{
    ...
    /// <summary>Gets a record by tenant and phone number, or <see langword="null"/> when unknown or expired.</summary>
    /// <param name="tenantId">The tenant id.</param>
    /// <param name="phoneNumber">The phone number.</param>
    /// <returns>The record or <see langword="null"/>.</returns>
    PendingPhoneSignupRecord? FindByPhoneNumber(string tenantId, string phoneNumber);
...
internal sealed class PendingPhoneSignup(TimeProvider timeProvider) : IPendingPhoneSignup
{
    ...
    public PendingPhoneSignupRecord? FindByPhoneNumber(string tenantId, string phoneNumber)
    {
        Sweep();
        foreach (var (id, entry) in _entries)
        {
            if (string.Equals(entry.TenantId, tenantId, StringComparison.Ordinal) &&
                string.Equals(entry.PhoneNumber, phoneNumber, StringComparison.Ordinal) &&
                timeProvider.GetUtcNow() < entry.RecordExpiresUtc)
            {
                return new PendingPhoneSignupRecord(id, entry.TenantId, entry.PhoneNumber);
            }
        }
        return null;
    }
