...
The `BuildAccountUiAssets` target in `Huia.OpenId.csproj` regenerates it as part of the build
...
```bash
dotnet build src/Huia.OpenId/Huia.OpenId.csproj -p:ForceAccountUiAssets=true
```
