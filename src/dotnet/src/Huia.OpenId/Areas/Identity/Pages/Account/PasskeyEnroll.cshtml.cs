    ...
    IStringLocalizer<SharedResource> localizer) : HuiaAccountPageModel
{
    ...
    private async Task<bool> IsPromptedAsync(HuiaUser user) =>
        await userManager.GetAuthenticationTokenAsync(user, HuiaOpenIdConstants.PasskeyLoginProvider, HuiaOpenIdConstants.EnrollPromptedTokenName) is not null;

    private Task MarkPromptedAsync(HuiaUser user) =>
        userManager.SetAuthenticationTokenAsync(user, HuiaOpenIdConstants.PasskeyLoginProvider, HuiaOpenIdConstants.EnrollPromptedTokenName, "1");
