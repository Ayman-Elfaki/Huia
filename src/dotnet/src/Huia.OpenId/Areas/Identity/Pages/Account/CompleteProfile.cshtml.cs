    ...
    TimeProvider timeProvider) : HuiaAccountPageModel
{
    ...
    public async Task<IActionResult> OnPostAsync()
    {
        ...
        if (state.ExternalProvider is { } registrationId)
        {
            ...
            claims.Add(new Claim(HuiaOpenIdConstants.ClaimTypes.ExternalIdp, registrationId));
            if (!string.IsNullOrEmpty(state.ExternalIdToken))
            {
                claims.Add(new Claim(HuiaOpenIdConstants.ClaimTypes.ExternalIdToken, state.ExternalIdToken));
            }
        }
        else
        {
