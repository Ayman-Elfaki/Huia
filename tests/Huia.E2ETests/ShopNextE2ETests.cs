...
/// <summary>
/// Drives the <c>Shop.Next</c>/<c>Shop.Api</c> sample end to end: the first Next.js consumer of
/// <c>next-huia-headless</c> and <c>Huia.Headless</c> together. Covers anonymous browsing, the
/// register/login JSON flow (pure headless identity — the app owns its own form), the protected cart page's
/// local redirect-to-login, full add-to-cart/checkout, phone SMS OTP, and external partner login.
/// </summary>
[Trait("Category", "E2E")]
...
public sealed class ShopNextE2ETests(NextFrontEndFixture fx)
{
    ...
    public async Task Anonymous_visitors_can_browse_products_but_are_prompted_to_sign_in_to_buy()
    {
        ...
        await page.GotoAsync($"{fx.ShopNextUrl}/");
        ...
        // The cart is a locally-protected route —
    ...
    public async Task Registers_signs_in_buys_something_and_signs_out()
    {
        ...
        // Lands back at "/" and the header shows the signed-in user.
        await page.WaitForURLAsync(u => u.TrimEnd('/') == fx.ShopNextUrl.TrimEnd('/'), new() { Timeout = 15_000 });
        await Expect(page.Locator("header")).ToContainTextAsync("Percy Shopper", new() { Timeout = 15_000 });
        ...
        // Allowed onto the cart page directly with the item in it.
        await page.GotoAsync($"{fx.ShopNextUrl}/cart");
        ...
        await Expect(page.Locator("body")).ToContainTextAsync("Order Confirmed!", new() { Timeout = 15_000 });
