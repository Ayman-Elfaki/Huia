...
```bash
# start the identity provider first (samples/Todo/Todo.IdentityServer), then:
dotnet run --project samples/Shared/Huia.Cli -- login
#   open the printed URL, sign in as admin@huia.local / Admin1!Pass, enter the code
dotnet run --project samples/Shared/Huia.Cli -- whoami
dotnet run --project samples/Shared/Huia.Cli -- scopes list --for todo
dotnet run --project samples/Shared/Huia.Cli -- logout
```
