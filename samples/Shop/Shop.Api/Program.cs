...
builder.Services
    .AddHuiaHeadless(huia =>
    {
        ...
        huia.UsePhoneLogin(phone =>
        {
            ...
            // repeatedly signing in and out with the same test number. Same fix Todo.IdentityServer's
        ...
        // Huia.External is the same mock upstream IdP Todo.Nuxt uses via Huia.OpenId's OpenIddict
        // client — here it's consumed through the classic OpenIdConnect handler instead, since
        // Huia.Headless has no OpenIddict dependency. See samples/Shared/Huia.External/Program.cs for the
...
if (builder.Environment.IsDevelopment() || enableE2E)
{
    ...
    // /e2e-otp — the same pattern Todo.IdentityServer uses.
